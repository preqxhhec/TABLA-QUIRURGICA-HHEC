    ...
    async function generarCampoEdicion(col, valor, rowKey, data) {
        ...
        if (col === 'Hora_de_entrada' || col === 'Hora_de_salida' || col === 'T_QX' || col === 'T_Qx' || col === 'T_Retraso') {
            return `<input type="time" data-field="${col}" class="edit-field edit-time" step="60" value="${valor}" style="width:100%; padding:6px 8px; border:1px solid #d1d9e6; border-radius:4px; font-size:0.85rem; background:white; box-sizing:border-box;">`;
        }
    ...
    function renderizarTablaLibro(registros, filtros = {}) {
        ...
        if (filtros.fechaInicio) {
            ...
            datosFiltrados = datosFiltrados.filter(r => {
                ...
                const fechaReg = new Date(partes[2], partes[1] - 1, partes[0]);
                return fechaReg >= fechaInicio;
            });
        }
        if (filtros.fechaFin) {
        ...
        let html = `
            <div style="margin-top:8px;">
                <div style="font-size:0.85rem; color:#64748b; margin-bottom:8px;">
                    Mostrando <strong>${datosFiltrados.length}</strong> de <strong>${registros.length}</strong> registros
                </div>
            ...
            datosFiltrados.forEach((item, index) => {
                ...
                html += `
                    <tr>
                        ...
                        <td>${item['1ra_Intervencion_Qx_Realizada'] || ''}</td>
                        <td>${item.DESTINO || ''}</td>
    ...
    async function mostrarDetalleLibro(key) {
        try {
            ...
            const contentDiv = document.getElementById('detalleLibroContent');
            const titleDiv = document.getElementById('detalleLibroTitle');
            const editarBtn = document.getElementById('editarRegistroBtn');
            const eliminarBtn = document.getElementById('eliminarRegistroBtn');
            const cerrarBtn = document.getElementById('cerrarDetalleLibro');
